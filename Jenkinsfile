pipeline {
     ...
     stages {
       ...
       stage('Test image') {
           ...
           steps {
              script {
                sh '''
                    curl --head http://localhost | grep "200"
                '''
              }
           }
      }
  ...
  post {
       success {
         slackSend (color: '#00FF00', message: "SUCCESSFULLY: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL}) - PROD URL => http://${PROD_APP_ENDPOINT} , STAGING URL => http://${STG_APP_ENDPOINT}")
         }
      failure {
